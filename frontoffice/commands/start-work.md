...
allowed-tools: Task, Grep, Glob, Read, Edit, MultiEdit, Write, Bash, TodoWrite, TodoRead, glab, git, mcp__atlassian__*, mcp__ide__*
---
...
- **Submodule**: `src/_core` - shared components (main branch: `master`, PR target: `QA-core`)

### Ecosystem Context
This is the **frontend** repository. Related repositories:
- **Backend APIs**:
  - `i2c_v2` - Core API at `/Users/camilolopez/DEVS/repos/i2c_v2`
  - `tangerine-api-v3` - Extended API at `/Users/camilolopez/DEVS/repos/tangerine-api-v3`
...
1. **Frontend-only tasks** (most common):
   ...
   - For submodule changes: create branches in BOTH repos (parent + submodule)
...
6. **After analyzing the task**, determine if changes will be in:
   ...
   - **Parent only**: Create branch in parent from `main`
...
7. Create feature branch(es):
   ...
   - Always in parent: `git checkout -b tan-XXXX-description`
...
- **Create comprehensive test suites** by:
  - Following testing guidelines and conventions detailed in `docs/testing/`
...
- **Use `/commit` command to create proper git commits with LINEAR references**
  - **IMPORTANT**: If changes are in `src/_core/`, commit in submodule FIRST, then commit submodule reference update in parent
